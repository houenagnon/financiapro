...
import type { TypePlacement } from "@/types/placement";
...
export default function TypesPlacementsPage() {
  const { data, loading, error, reload } = useApi<Paginated<TypePlacement>>(
    "/types-placements/",
  );
  ...
  const onSubmit = async (values: TypeFormValues) => {
    setServerError(null);
    try {
      await api("/types-placements/", { method: "POST", body: values });
      reset();
      reload();
    } catch (err) {
  ...
  const toggle = async (type: TypePlacement) => {
    await api(`/types-placements/${type.id}/`, {
      method: "PATCH",
  ...
  return (
    <div>
      ...
      >
        <div className="min-w-40 flex-1">
          <label className="block text-sm font-medium text-slate-700">
            ...
            <input
              type="text"
              placeholder="Dépôt à terme, Action…"
              {...register("libelle")}
      ...
      {data && data.results.length === 0 && (
        <EmptyMessage message="Aucun type de placement défini." />
      )}
      {data && data.results.length > 0 && (
